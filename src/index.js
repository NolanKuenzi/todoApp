...
class MyTodoApp extends React.Component {
  ...
  render() {
    ...
    let Update;
    if (this.state.edit === true) {
      Update = <form id="updateForm" className="rosterPlayers" onSubmit={this.updateItem}><input id="updateInput" value={this.state.updateLi} onChange={this.handleInput}></input> <br /><button id="updateButton">Update</button><button id="closeButton">X</button></form>;
    } else {
      Update = null;
    }
    return (
